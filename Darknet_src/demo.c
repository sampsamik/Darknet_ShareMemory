...
//ShareMemory
static int share_memory;

//ShareMemory modification
void *fetch_in_thread(void *ptr)
{
    while (!custom_atomic_load_int(&flag_exit)) {
        ...
        else if (share_memory)//ShareMemory
        {
            in_s = get_image_from_stream_resize_SM(net.w, net.h, net.c, &in_img, dont_close_stream);
        }
        else
...
void demo(char *cfgfile, char *weightfile, float thresh, float hier_thresh, int cam_index, const char *filename, char **names, int classes, int avgframes,
    ...
    int benchmark, int benchmark_layers, int share_mem)
{
    ...
    {
        printf("________________________________SHARED MEMORY FUNCTIONS RUNNING________________________________\n");
    }
    else {
    ...
    while (1) {
        ++count;
        {
            ...
            if (share_mem)
            {
                printf("FPS:%.1f \t AVG_FPS:%.1f \t Count:%i\r", fps, avg_fps, count);
                //Only send coordinates, no need to show from here
                send_detections_cv_v3_SM(local_dets, local_nboxes, demo_thresh, demo_names, demo_alphabet, demo_classes, demo_ext_output);
            }
            else if (!benchmark && !dontdraw_bbox) draw_detections_cv_v3(show_img, local_dets, local_nboxes, demo_thresh, demo_names, demo_alphabet, demo_classes, demo_ext_output);

...
#define BUF_SIZE 4800    //100 objects per image
    TCHAR szName[] = TEXT("ShareStringMemory");

    //std::string strMapName("ShareStringMemory");
    //LPVOID pBufferString;
    ...
    void writeString2ShareMemory(std::string strValue)
    {
        memcpy((char*)pBufString, strValue.c_str(), BUF_SIZE);
    }
    ...
    void readStringFromShareMemory(char* STRING_Address, std::string& strValue)
    {
        memcpy(&strValue, STRING_Address, sizeof(strValue));
    }
    ...
    extern "C" mat_cv* get_capture_frame_cv_SM(bool &exit_) {
        static bool initSharedMemory = 1;
    ...
    //ShareMemory
    bool exit_ = 0;
    extern "C" image get_image_from_stream_resize_SM(int w, int h, int c, mat_cv** in_img, int dont_close)
    {
        c = c ? c : 3;
        cv::Mat *src = NULL;
        ...
        do {
            if (src) delete src;
            src = (cv::Mat*)get_capture_frame_cv_SM(exit_);
            if (exit_)
            {
    ...
    //SharMemory
    extern "C" void send_detections_cv_v3_SM(detection *dets, int num, float thresh, char **names, image **alphabet, int classes, int ext_output)
    {
        std::string obj_coord;
        try {
            int i, j;
            static int frame_id = 0;
            frame_id++;
            ...
            for (i = 0; i < num; ++i) {
                ...
                if (class_id >= 0) {
                    ...
                    obj_coord += std::to_string(class_id)
                        + "-"
                        + std::to_string(b.w)
                        + "-"
                        + std::to_string(b.h)
        ...
        writeString2ShareMemory(obj_coord);

        //Check flag to see if error has occurred in other program and changed the flag
        char flag;
        readFlagFromShareMemory(FLAG_ADDRESS, flag);
        if (3 != flag)
            writeFlag2ShareMemory(FLAG_ADDRESS, 2);
    }
